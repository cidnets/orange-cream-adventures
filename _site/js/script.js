...
document.addEventListener('DOMContentLoaded', () => {
  const menuToggle = document.getElementById('navbar-toggle');
  const navContainer = document.querySelector('.nav-container'); // You may need to adjust this selector

  document.addEventListener('click', (event) => {
    // Check if the menu is open and if the click was outside the nav container and toggle button
    if (menuToggle && menuToggle.checked) {
      // Use .contains() to check if the click target is inside the nav container
      const isClickInsideNav = navContainer && navContainer.contains(event.target);
      const isClickOnToggle = menuToggle.contains(event.target);

      // If the click was NOT inside the nav container and NOT on the toggle button itself, close the navbar
